function [ organ_subskeleton] = SkeletonDecompositionOld(mainskeleton,spls, joints,roots, spls_adj,corresp,features,show_results )
...
function visualize_subskeleton(spls, organ_skeleton, joints, roots)
    ...
    for idx = 1:organ_num
        ...
        if idx == organ_num  % 主茎加粗
            ...
            plot3(spls(path, 1), spls(path, 2), spls(path, 3), '-', 'Color', color(idx, :), 'LineWidth', 1);
        end
